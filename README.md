...
resourceStringToBaseUnit("10P"); // BigInt(10000000000000000)
```

> [!TIP]
> The function returns a `number` or `bigint` for a very large numbers. If you know for sure, that your resources will never reach a value larger than ~10e16 (a really huge 10P CPU or 8Pi Mem), then the BigIng might be an unnecessary complication and you can safely convert all the results to `number` type either via TypeScript only by re-assigning the function to a type that returns only `number`:
> `const resourceStringToBaseUnitNumber = resourceStringToBaseUnit as ((s: string) => number)`
> or convert the return to a `number` in runtime via higher-order function:
> `const resourceStringToBaseUnitNumber = (s: string): number => Number(resourceStringToBaseUnit(s));`
> The former has zero runtime overhead, the latter is safer in case of unexpected large values, where your logic will not crash on arithmetics with mixed number and bigint, but loss of precision is possible in that case.
...
// With specific unit
formatBaseUnitCpu(1.5, { unit: "m" }); // "1500m"
